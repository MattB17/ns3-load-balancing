...
Ipv4DrbHelper* Ipv4DrbHelper::Copy() const {
	return new Ipv4DrbHelper(*this);
}

Ptr<Ipv4Drb> Ipv4DrbHelper::Create(Ptr<Node> node) const {
	return CreateObject<Ipv4Drb>();
}

Ptr<Ipv4Drb> Ipv4DrbHelper::GetIpv4Drb(Ptr<Ipv4> ipv4) const {
	Ptr<Ipv4RoutingProtocol> ipv4rp = ipv4->GetRoutingProtocol();
	// DRB is not a complete routing protocol so it must be specified
	// as part of a List Routing protocol.
